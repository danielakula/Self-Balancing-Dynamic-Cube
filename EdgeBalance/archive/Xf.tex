...
void taskLQR(void *pvParameters) {
    ...
    for(;;) {
        ...
        if (currentFaults == FAULT_NONE) {
            ...
            if (std::abs(imu.getPitch()) > 0.35f) { // ~20 degrees
                target = 0.0f; 
                lqr.resetFilter(); // Purge 'xf' when the cube falls over!
            } else {
                // Pass the RAW angle directly into the compute function.
                // The LQR class handles the filtering and un-biasing internally.
                target = -lqr.compute(
                    imu.getPitch(),         // x1: RAW Cube Angle
