...
// Instantiate globally with specific SPI pins
...
void taskFOC(void * pvParameters) {
    ...
    // Kill the Watchdog on Core 1 
    disableCore1WDT(); 

    // Set this specific task to the highest possible FreeRTOS priority
    vTaskPrioritySet(NULL, configMAX_PRIORITIES - 1);

    for(;;) {
        // The Spinlock Handshake
...
void taskLQR(void *pvParameters) {
    ...
    for(;;) {
        ...
        // =======================================================
        spiRequested.store(true); // Ask Core 1 for the bus
        
        while (!spiSafeToUse.load()) { 
            // Wait until Core 1 finishes its math and grants access
            delayMicroseconds(1); 
        }

        // safe access to the SPI hardware
        ...
        // Safely grab the wheel velocity from Core 0
        float current_wheel_vel = shared_motor_velocity.load();
        
        // Pass states to the LQR Controller
        if (currentFaults == FAULT_NONE) {
          target = -lqr.compute(
        ...
        // Safely send the torque command to Core 0
