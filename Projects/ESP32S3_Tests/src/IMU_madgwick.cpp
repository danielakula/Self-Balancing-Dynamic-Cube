...
const float BASE_BETA = 0.01f;     // You can raise this slightly now that it's adaptive
...
void calibrate() {
  ...
  // Let sensor warm up for a moment before sampling
  delay(500);
...
void loop() {
  ...
  if ((now - lastFilter) >= FILTER_US) {
    ...
    // 2. Pure Accel Data (I have temporarily restored your bias from the original code)
    float ax = accel.acceleration.x - ax_bias;
    float ay = accel.acceleration.y - ay_bias;
    float az = accel.acceleration.z;  

    // 3. Adaptive Beta Logic (Using pure magnitudes)
    ...
    // 4. THE FIX: Feed the pure, matching axes into the filter
