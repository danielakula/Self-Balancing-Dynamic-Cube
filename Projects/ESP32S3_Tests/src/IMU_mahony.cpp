...
#include <SPI.h>
#include <MahonyAHRS.h> // Using your new local library
...
const float GRAVITY = 9.80665f;   // Earth's gravity in m/s^2
const float ACCEL_TOLERANCE = 1.5f; // Ignore accel if magnitude is outside 9.81 +/- 1.5
...
void loop() {
  ...
  if ((now - lastFilter) >= FILTER_US) {
    // 1. Calculate EXACT dt in seconds
    ...
    // 6. Update Filter with exact dt
    // Keeping your custom axis orientation (gx, gz, -gy)
    filter.update(gx, gy, gz, ax, ay, az, dt);

    // 7. Low pass filter (Optional, Mahony is usually smooth enough without this)
