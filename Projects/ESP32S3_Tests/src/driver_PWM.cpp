...
// --- SPI HELPERS (Must be above setup) ---
...
void setup() {
    ...
    // 2. Initialize SPI for ESP32-S3
    SPI.begin(DRV_SCK, DRV_MISO, DRV_MOSI, DRV_CS); 
    SPI.beginTransaction(SPISettings(1000000, MSBFIRST, SPI_MODE1));

    // 3. APPLY YOUR BINARY REGISTERS
...
void loop() {
    ...
    // B. Open-Loop Sine (The "SimpleFOC-Manual" way)
    static float angle = 0;
    float v_test = 5.0; // Target voltage strength

    // Standard 120-degree phase shift math
