...
// 1. FIX: Use the specific AS5047 profile so SPI Mode 1 is applied
...
void MotorTask(void *pvParameters) {
    for (;;) {
        ...
        // 3. FIX: Give the AS5047 chip a tiny breather between SPI reads
        // 1 tick = 1 millisecond. This gives us a highly stable 1000Hz read rate.
...
void setup() {
    ...
    Serial.begin(115200);
    delay(2000); // 2-second Native USB catchup delay!
    Serial.println("\n--- AS5047 SPI Test Booting ---");

    pinMode(48, OUTPUT); // Your Master DRV_CS pin
    digitalWrite(48, HIGH);
    pinMode(43, OUTPUT);

    // 2. FIX: Pass -1 so the ESP32 OS doesn't fight SimpleFOC for the CS pin
