...
#define LED_PIN      43  // Your LED pin from earlier
...
void setup() {
    ...
    // 1. Setup the 3.3V supply pin
    pinMode(KILL_OUT_PIN, OUTPUT);
    digitalWrite(KILL_OUT_PIN, HIGH);

    // 2. Setup the sensing pin with the internal pulldown resistor
    pinMode(KILL_IN_PIN, INPUT_PULLDOWN);

    // 3. Setup the LED indicator
...
void loop() {
    ...
    // If the state has changed since the last time we checked...
    ...
    // A tiny 50ms delay acts as a "debounce" so physical switch 
    // vibrations don't trigger multiple rapid ON/OFF prints
    delay(50); 
}